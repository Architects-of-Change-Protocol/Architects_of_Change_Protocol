...
function resolveExecutionTarget(input) {
    ...
    return {
        ...
        operation: typeof record.operation === 'string' && record.operation.trim() !== '' ? record.operation.trim() : 'unknown',
    };
}
export function authorizeExecution(request) {
    const now = request.now ?? new Date();
    const fallbackTarget = resolveExecutionTarget(request.execution_target);
