...
function toScopeKey(entry) {
    return `${entry.type}:${entry.ref}`;
}
function findScopeMismatch(normalizedRequest, allowedScope) {
...
function findPermissionMismatch(normalizedRequest, allowedPermissions) {
...
export function evaluateEnforcement(request) {
    ...
    try {
        ...
        return buildDenyDecision(ENFORCEMENT_REASON_CODES.REQUEST_INVALID, [error instanceof Error ? error.message : 'Enforcement request parsing failed.'], { now });
    }
    ...
    if (scopeMatch.mismatch !== undefined) {
        return buildDenyDecision(ENFORCEMENT_REASON_CODES.SCOPE_NOT_ALLOWED, [`Requested scope is not allowed: ${scopeMatch.mismatch.type}:${scopeMatch.mismatch.ref}.`], {
            now,
            normalized_request: normalizedRequest,
            normalized_capability: normalizedCapability,
    ...
    if (permissionMatch.mismatch !== undefined) {
        return buildDenyDecision(ENFORCEMENT_REASON_CODES.PERMISSION_NOT_ALLOWED, [`Requested permission is not allowed: ${permissionMatch.mismatch}.`], {
            now,
            normalized_request: normalizedRequest,
            normalized_capability: normalizedCapability,
            matched_scope: scopeMatch.matched,
    ...
    if (normalizedRequest.subject !== undefined &&
        normalizedRequest.subject !== normalizedCapability.subject) {
        return buildDenyDecision(ENFORCEMENT_REASON_CODES.SUBJECT_MISMATCH, ['Request subject does not match capability subject.'], {
            now,
            normalized_request: normalizedRequest,
            normalized_capability: normalizedCapability,
            matched_scope: scopeMatch.matched,
    ...
    if (normalizedRequest.grantee !== undefined &&
        normalizedRequest.grantee !== normalizedCapability.grantee) {
        return buildDenyDecision(ENFORCEMENT_REASON_CODES.GRANTEE_MISMATCH, ['Request grantee does not match capability grantee.'], {
            now,
            normalized_request: normalizedRequest,
            normalized_capability: normalizedCapability,
            matched_scope: scopeMatch.matched,
    ...
    if (normalizedRequest.marketMakerId !== undefined &&
        normalizedRequest.marketMakerId !== (normalizedCapability.marketMakerId ?? undefined)) {
        return buildDenyDecision(ENFORCEMENT_REASON_CODES.MARKET_MAKER_MISMATCH, ['Request market maker binding does not match capability marketMakerId.'], {
            now,
            normalized_request: normalizedRequest,
            normalized_capability: normalizedCapability,
            matched_scope: scopeMatch.matched,
    ...
    if (!accessAllowed) {
        return buildDenyDecision(ENFORCEMENT_REASON_CODES.REQUEST_INVALID, ['Capability access evaluation denied the request.'], {
            now,
            normalized_request: normalizedRequest,
            normalized_capability: normalizedCapability,
            matched_scope: scopeMatch.matched,
