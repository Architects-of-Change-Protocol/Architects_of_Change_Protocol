...
function parseStringField(record, field) {
...
function parseDateField(record, field) {
...
function parsePermissions(record) {
    ...
    return permissions.map((permission, index) => {
        ...
        if (!PERMISSION_PATTERN.test(normalized)) {
            throw new CapabilityParseError(`Capability permission ${index} must be lowercase alphanumeric with hyphens.`);
        }
        return normalized;
    });
}
...
export function toCanonicalCapabilityHashPayload(capability) {
    return {
...
export function computeCapabilityDeterministicHash(capability) {
