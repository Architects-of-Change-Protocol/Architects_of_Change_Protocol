...
const itemConfidence = (item: ProjectDiscoveryItem): number => item.confidence === undefined ? 65 : clampScore(item.confidence);
const evidenceRefs = (items: ProjectDiscoveryItem[]): string[] => Array.from(new Set(items.flatMap((item) => item.evidenceSourceRefs)));
const itemRefs = (items: ProjectDiscoveryItem[]): string[] => items.map((item) => item.id);
const trace = (items: ProjectDiscoveryItem[]): GovernanceTrace => ({ discoveryItemRefs: itemRefs(items), evidenceSourceRefs: evidenceRefs(items), confidence: clampScore(average(items.map(itemConfidence))) });
const hasItems = (items?: ProjectDiscoveryItem[]): items is ProjectDiscoveryItem[] => Array.isArray(items) && items.length > 0;

function defaultBriefId(projectId: string, discoveryVersion: number, briefVersion: number): string {
  return `${projectId}:discovery-${discoveryVersion}:brief-${briefVersion}`;
}

function pressureFromCount(count: number, breakpoints: Array<[number, number]>): number {
  const sorted = [...breakpoints].sort((a, b) => a[0] - b[0]);
  for (let index = 0; index < sorted.length; index += 1) {
    const [limit, score] = sorted[index];
    if (count <= limit) return score;
  }
  return sorted[sorted.length - 1]?.[1] ?? 0;
}

function deriveStakeholderPressure(discovery: ProjectDiscoverySnapshot): number {
...
function deriveDependencyPressure(discovery: ProjectDiscoverySnapshot): number {
...
function deriveRiskPressure(discovery: ProjectDiscoverySnapshot): number {
...
function deriveUnknownPressure(discovery: ProjectDiscoverySnapshot): number {
...
function deriveComplexity(discovery: ProjectDiscoverySnapshot, stakeholderPressure: number, dependencyPressure: number, riskPressure: number): ExecutionComplexity {
...
function deriveHealth(dependencyPressure: number, riskPressure: number, unknownPressure: number, discovery: ProjectDiscoverySnapshot): ExecutionHealth {
  ...
  if (unknownPressure >= 75 || riskPressure >= 80 || dependencyPressure >= 85 || (missingCriticalOwnership && unknownPressure >= 55) || blockedRisk) return "Red";
  if (unknownPressure >= 35 || riskPressure >= 35 || dependencyPressure >= 35 || missingCriticalOwnership) return "Yellow";
  return "Green";
}

function deriveConfidence(discovery: ProjectDiscoverySnapshot): number {
...
function allItems(discovery: ProjectDiscoverySnapshot): ProjectDiscoveryItem[] {
...
function findingFromItem(item: ProjectDiscoveryItem, severity: GovernanceFinding["severity"], description: string): GovernanceFinding {
  return { title: item.label, description, severity, ...trace([item]) };
}

function fallbackTrace(discovery: ProjectDiscoverySnapshot): GovernanceTrace {
...
function firstAvailableTrace(discovery: ProjectDiscoverySnapshot): GovernanceTrace {
  const firstItem = allItems(discovery).slice(0, 1);
  return hasItems(firstItem) ? trace(firstItem) : fallbackTrace(discovery);
}

function buildCriticalFindings(discovery: ProjectDiscoverySnapshot): GovernanceFinding[] {
  return [
...
function buildPositiveSignals(discovery: ProjectDiscoverySnapshot): GovernanceFinding[] {
  const signals: GovernanceFinding[] = [];
  if (hasItems(discovery.governanceRoles)) signals.push({ title: "Governance roles identified", description: "Discovery contains named governance ownership or decision roles.", severity: "positive", ...trace(discovery.governanceRoles) });
  if (hasItems(discovery.deliverables)) signals.push({ title: "Deliverables documented", description: "Discovery contains structured delivery outputs for execution planning.", severity: "positive", ...trace(discovery.deliverables) });
  if (hasItems(discovery.milestones)) signals.push({ title: "Milestones documented", description: "Discovery contains milestone evidence that can anchor execution cadence.", severity: "positive", ...trace(discovery.milestones) });
  if (hasItems(discovery.dependencies)) signals.push({ title: "Dependencies documented", description: "Execution dependencies are visible rather than implicit.", severity: "positive", ...trace(discovery.dependencies) });
  if (hasItems(discovery.acceptanceCriteria)) signals.push({ title: "Acceptance criteria detected", description: "Acceptance criteria were found in available discovery evidence.", severity: "positive", ...trace(discovery.acceptanceCriteria) });
  return signals.slice(0, 5);
}

function buildConcerns(discovery: ProjectDiscoverySnapshot): GovernanceFinding[] {
  const concerns: GovernanceFinding[] = [];
  if (!discovery.governanceRoles?.some((role) => /sponsor/i.test(role.label))) concerns.push({ title: "Project sponsor not detected", description: "Discovery did not identify a project sponsor, creating an ownership concern.", severity: "concern", ...(hasItems(discovery.governanceRoles) ? trace(discovery.governanceRoles) : fallbackTrace(discovery)) });
  if (!hasItems(discovery.acceptanceCriteria)) concerns.push({ title: "Acceptance criteria not detected", description: "Discovery did not locate acceptance criteria in available evidence.", severity: "concern", ...firstAvailableTrace(discovery) });
  if (!hasItems(discovery.operationalSupport)) concerns.push({ title: "Operational support model undefined", description: "Discovery did not locate post-deployment support ownership or transition details.", severity: "concern", ...firstAvailableTrace(discovery) });
...
function buildRecommendedActions(discovery: ProjectDiscoverySnapshot, concerns: GovernanceFinding[], criticalFindings: GovernanceFinding[]): RecommendedGovernanceAction[] {
  const actions: RecommendedGovernanceAction[] = [];
  const addAction = (action: string, rationale: string, priority: RecommendedGovernanceAction["priority"], source: GovernanceTrace): void => {
    if (!actions.some((existing) => existing.action === action)) actions.push({ action, rationale, priority, ...source });
  };
  criticalFindings.forEach((finding) => addAction(`Resolve governance path for ${finding.title}.`, finding.description, "urgent", finding));
  if (!discovery.governanceRoles?.some((role) => /sponsor/i.test(role.label))) addAction("Validate project sponsor ownership.", "Sponsor ownership was not detected in discovery evidence.", "high", firstAvailableTrace(discovery));
  if (!hasItems(discovery.acceptanceCriteria)) addAction("Confirm acceptance criteria and acceptance owner.", "Acceptance criteria were not detected in discovery evidence.", "high", firstAvailableTrace(discovery));
  if (!hasItems(discovery.operationalSupport)) addAction("Define support transition plan.", "Operational support ownership was not detected in discovery evidence.", "medium", firstAvailableTrace(discovery));
  ...
  if (actions.length < 3 && hasItems(discovery.stakeholders)) addAction("Confirm stakeholder decision cadence.", "Stakeholders were identified and should have an explicit governance cadence.", "medium", trace(discovery.stakeholders));
  if (actions.length < 3 && hasItems(discovery.deliverables)) addAction("Validate ownership for documented deliverables.", "Deliverables were detected and should have accountable owners before execution proceeds.", "medium", trace(discovery.deliverables));
  if (actions.length < 3 && hasItems(discovery.milestones)) addAction("Confirm milestone acceptance gates.", "Milestones were detected and should be tied to acceptance or governance checkpoints.", "medium", trace(discovery.milestones));
  if (actions.length < 3) addAction("Review discovery evidence with the project manager.", "Available discovery evidence should be validated before relying on the bootstrap governance assessment.", "medium", fallbackTrace(discovery));
  return actions.slice(0, 7);
}

function buildSummary(brief: Omit<ProjectBootstrapBrief, "governanceSummary">): string {
...
export class BootstrapGovernanceBriefEngine {
  constructor(private readonly logger?: GovernanceBriefLogger) {}
...
export class InMemoryProjectBootstrapBriefRepository implements ProjectBootstrapBriefRepository {
  private readonly briefs: ProjectBootstrapBrief[] = [];

  async save(brief: ProjectBootstrapBrief): Promise<void> {
  ...
  async latest(projectId: string, workspaceId: string): Promise<ProjectBootstrapBrief | undefined> {
  ...
  async history(projectId: string, workspaceId: string): Promise<ProjectBootstrapBrief[]> {
...
export async function getProjectBootstrapBrief(repository: ProjectBootstrapBriefRepository, authorization: ProjectReadAuthorization, request: GetProjectBootstrapBriefRequest): Promise<GetProjectBootstrapBriefResponse> {
  const allowed = await authorization.canReadProject(request.projectId, request.workspaceId, request.actorId);
  if (!allowed) return { status: 403, error: "Project read access is required." };
  const brief = await repository.latest(request.projectId, request.workspaceId);
  if (!brief) return { status: 404, error: "Project bootstrap brief was not found." };
...
export function toGovernanceSnapshotCard(brief: ProjectBootstrapBrief): GovernanceSnapshotCard {
  return {
...
export async function generateAndStoreProjectBootstrapBrief(repository: ProjectBootstrapBriefRepository, engine: BootstrapGovernanceBriefEngine, discovery: ProjectDiscoverySnapshot, options: Omit<BootstrapGovernanceBriefOptions, "latestBriefVersion"> = {}): Promise<ProjectBootstrapBrief> {
