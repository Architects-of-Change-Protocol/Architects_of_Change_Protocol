...
exports.PORTABLE_SOVEREIGN_CLAIM_ARTIFACT_KINDS = exports.PORTABLE_SOVEREIGN_MANIFEST_ARTIFACT_KINDS = exports.SOVEREIGNTY_PORTABILITY_REASON_CODES = exports.SOVEREIGNTY_PORTABILITY_BUNDLE_SCHEMA_VERSION = void 0;
exports.portableManifestOf = portableManifestOf;
exports.portableClaimOf = portableClaimOf;
exports.isPortableSovereignManifestArtifact = isPortableSovereignManifestArtifact;
exports.isPortableSovereignClaimArtifact = isPortableSovereignClaimArtifact;
exports.validateSovereigntyPortabilityBundleV1 = validateSovereigntyPortabilityBundleV1;
exports.isValidSovereigntyPortabilityBundleV1 = isValidSovereigntyPortabilityBundleV1;
exports.tryBuildSovereigntyPortabilityBundleV1 = tryBuildSovereigntyPortabilityBundleV1;
exports.buildSovereigntyPortabilityBundleV1 = buildSovereigntyPortabilityBundleV1;
const canonical_1 = require("../canonical");
const standing_1 = require("../claims/standing");
 ...
 */
exports.SOVEREIGNTY_PORTABILITY_BUNDLE_SCHEMA_VERSION = 'aoc-sovereignty-portability-bundle/1';
/**
 ...
 */
exports.SOVEREIGNTY_PORTABILITY_REASON_CODES = Object.freeze({
...
exports.PORTABLE_SOVEREIGN_MANIFEST_ARTIFACT_KINDS = Object.freeze([
    'manifest',
    'signed-manifest',
]);
/** The claim artifact wrapper kinds this bundle version understands. */
exports.PORTABLE_SOVEREIGN_CLAIM_ARTIFACT_KINDS = Object.freeze(['claim', 'signed-claim']);
 ...
 */
function isStructurallyRepresentableProof(value) {
    if (!isPlainObject(value))
        return false;
    return (isNonBlankString(value.algorithm)
...
function isPortableSovereignClaimValue(value) {
    return (0, manifest_1.isValidOriginClaim)(value) || (0, manifest_1.isValidAuthorityClaim)(value) || (0, manifest_1.isValidDerivationClaim)(value);
}
/** The manifest a manifest artifact carries, whichever wrapper kind it uses. */
function portableManifestOf(artifact) {
...
function portableClaimOf(artifact) {
    return artifact.kind === 'claim' ? artifact.claim : artifact.signedClaim.claim;
}
function isPortableSovereignManifestArtifact(value) {
    if (!isPlainObject(value))
        return false;
    if (value.kind === 'manifest') {
        return (hasExactKeys(value, ['kind', 'manifest'])
            && isPlainObject(value.manifest)
            && (0, manifest_1.validateSovereignManifestV1)(value.manifest).valid);
    }
    if (value.kind === 'signed-manifest') {
        ...
        return (isPlainObject(signed.manifest)
            && (0, manifest_1.validateSovereignManifestV1)(signed.manifest).valid
            && isNonBlankString(signed.manifestDigest)
            && isStructurallyRepresentableProof(signed.proof));
    }
    return false;
}
function isPortableSovereignClaimArtifact(value) {
    if (!isPlainObject(value))
        return false;
    if (value.kind === 'claim') {
    ...
    if (value.kind === 'signed-claim') {
        ...
        return (isPortableSovereignClaimValue(signed.claim)
            && isNonBlankString(signed.digest)
            && isStructurallyRepresentableProof(signed.proof));
    }
    return false;
}
function isKnownManifestArtifactKind(value) {
    return (isPlainObject(value)
        && exports.PORTABLE_SOVEREIGN_MANIFEST_ARTIFACT_KINDS.includes(value.kind));
}
function isKnownClaimArtifactKind(value) {
    return (isPlainObject(value)
        && exports.PORTABLE_SOVEREIGN_CLAIM_ARTIFACT_KINDS.includes(value.kind));
}
/**
 ...
 */
function toBundleSubject(subject) {
    return Object.freeze({
        sovereignAssetId: subject.sovereignAssetId,
        ...(subject.externalReference === undefined
            ? {}
            : { externalReference: Object.freeze((0, identity_1.buildSovereignExternalReference)(subject.externalReference)) }),
    });
}
function isCanonicalBundleSubject(value) {
    if (!isPlainObject(value) || !(0, identity_1.isValidSovereignSubjectRef)(value))
        return false;
    if (!hasExactKeys(value, hasOwn(value, 'externalReference') ? ['sovereignAssetId', 'externalReference'] : ['sovereignAssetId'])) {
        return false;
    }
 ...
 */
function validateSovereigntyPortabilityBundleV1(value) {
    const codes = exports.SOVEREIGNTY_PORTABILITY_REASON_CODES;
    if (!isPlainObject(value)) {
    ...
    if (value.schemaVersion !== exports.SOVEREIGNTY_PORTABILITY_BUNDLE_SCHEMA_VERSION) {
    ...
    if (!Array.isArray(standings)) {
        ...
        for (const standing of Array.from(standings)) {
            if (!(0, standing_1.isValidCanonicalStanding)(standing)) {
                reasons.push(codes.invalidStanding);
                continue;
            }
...
function isValidSovereigntyPortabilityBundleV1(value) {
...
function assembleBundle(input) {
    const manifests = [...(input.manifests ?? [])].sort((a, b) => portableManifestOf(a).manifestVersion - portableManifestOf(b).manifestVersion);
    const claims = [...(input.claims ?? [])].sort((a, b) => compareStrings(portableClaimOf(a).id, portableClaimOf(b).id));
    const standings = [...(input.standings ?? [])].sort((a, b) => compareStrings(a.id, b.id));
    return Object.freeze({
        schemaVersion: exports.SOVEREIGNTY_PORTABILITY_BUNDLE_SCHEMA_VERSION,
 ...
 */
function tryBuildSovereigntyPortabilityBundleV1(input) {
    const codes = exports.SOVEREIGNTY_PORTABILITY_REASON_CODES;
    ...
    for (const artifact of Array.from(input.manifests ?? [])) {
        if (!isKnownManifestArtifactKind(artifact))
            return { valid: false, reasons: [codes.unsupportedArtifactKind] };
        if (!isPortableSovereignManifestArtifact(artifact)) {
    ...
    for (const standing of Array.from(input.standings ?? [])) {
        if (!(0, standing_1.isValidCanonicalStanding)(standing))
            return { valid: false, reasons: [codes.invalidStanding] };
    }
 ...
 */
function buildSovereigntyPortabilityBundleV1(input) {
