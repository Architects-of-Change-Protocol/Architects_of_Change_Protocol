...
export const INTEGRITY_SOVEREIGNTY_CAPABILITY_REASON_CODES = Object.freeze({
...
export type IntegritySovereigntyCapabilityReasonCode =
  (typeof INTEGRITY_SOVEREIGNTY_CAPABILITY_REASON_CODES)[keyof typeof INTEGRITY_SOVEREIGNTY_CAPABILITY_REASON_CODES];
 ...
 */
export const INTEGRITY_SOVEREIGNTY_CAPABILITY_OPERATIONS = Object.freeze([
  'compute-content-identity',
  'verify-content-identity',
  'compute-manifest-digest',
] as const);

export type IntegritySovereigntyCapabilityOperation =
  (typeof INTEGRITY_SOVEREIGNTY_CAPABILITY_OPERATIONS)[number];
...
export type IntegritySovereigntyCapabilityInput =
...
export type IntegritySovereigntyCapabilityOutput =
 ...
 */
function isManifestShaped(value: unknown): value is SovereignManifestV1 {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return false;
  return Array.isArray((value as { authorityClaims?: unknown }).authorityClaims);
}
 ...
 */
export function validateIntegritySovereigntyCapabilityInput(
  value: unknown,
): IntegritySovereigntyCapabilityInputValidationResult {
  const codes = INTEGRITY_SOVEREIGNTY_CAPABILITY_REASON_CODES;
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    return { valid: false, reasons: [codes.invalidInput] };
  }

  const candidate = value as { operation?: unknown; bytes?: unknown; expected?: unknown; manifest?: unknown };
  const operation = candidate.operation;

  if (
    typeof operation !== 'string'
    || !(INTEGRITY_SOVEREIGNTY_CAPABILITY_OPERATIONS as readonly string[]).includes(operation)
  ) {
    return { valid: false, reasons: [codes.unsupportedOperation] };
  }

  const reasons: IntegritySovereigntyCapabilityReasonCode[] = [];
  ...
  if (operation === 'verify-content-identity' && !isValidContentIdentity(candidate.expected)) {
    reasons.push(codes.invalidExpectedContentIdentity);
  }

  if (operation === 'compute-manifest-digest') {
    if (!isManifestShaped(candidate.manifest) || !validateSovereignManifestV1(candidate.manifest).valid) {
...
export function isValidIntegritySovereigntyCapabilityInput(
  value: unknown,
): value is IntegritySovereigntyCapabilityInput {
  return validateIntegritySovereigntyCapabilityInput(value).valid;
}

export interface IntegritySovereigntyCapabilityImplementation
 ...
 */
export function createIntegritySovereigntyCapabilityImplementation(): IntegritySovereigntyCapabilityImplementation {
  const capability: SovereigntyCapabilityRef = requireSovereigntyCapabilityRef('integrity');

  return Object.freeze({
    capability,
    async invoke(
      invocation: SovereigntyCapabilityInvocation<IntegritySovereigntyCapabilityInput>,
    ): Promise<SovereigntyCapabilityExecutionOutcome<IntegritySovereigntyCapabilityOutput>> {
      ...
      switch (input.operation) {
        ...
        case 'verify-content-identity': {
          ...
          const check = verifyContentIdentity(input.bytes, input.expected);
          return {
            status: 'succeeded',
            output: Object.freeze({
              operation: 'verify-content-identity' as const,
              check: Object.freeze({
                valid: check.valid,
        ...
        case 'compute-manifest-digest':
          return {
            status: 'succeeded',
            output: Object.freeze({
              operation: 'compute-manifest-digest' as const,
