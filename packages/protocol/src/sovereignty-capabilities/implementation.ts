 ...
 */
export type SovereigntyCapabilityExecutionOutcome<TOutput> =
  | SovereigntyCapabilitySuccessOutcome<TOutput>
...
export interface SovereigntyCapabilityImplementation<TInput, TOutput> {
  ...
  ): Promise<SovereigntyCapabilityExecutionOutcome<TOutput>>;
}
 ...
 * legitimate result.
 */
export function isValidSovereigntyCapabilityExecutionOutcome(
  value: unknown,
): value is SovereigntyCapabilityExecutionOutcome<unknown> {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    return false;
  }
  const candidate = value as Partial<SovereigntyCapabilityExecutionOutcome<unknown>>;
  ...
  if (candidate.status === 'failed') {
    const { reasonCodes } = candidate as SovereigntyCapabilityFailureOutcome;
    if (
