...
import { LICENSING_TERMS_REASON_CODES, type LicensingTermsReasonCode } from './reason-codes';
import {
  ...
  type SovereignLicenseTermsRuleEffect,
  type SovereignLicenseTermsRuleV1,
  type SovereignLicenseTermsV1,
} from './terms';
import { AOC_LICENSING_SEMANTIC_NAMESPACE, isAocLicensingActionTermId } from './vocabulary';
 ...
 */
function distinct(reasons: readonly LicensingTermsReasonCode[]): readonly LicensingTermsReasonCode[] {
 ...
 */
function isStructuralPrincipal(value: unknown): boolean {
...
export function validateSovereignLicenseTermsAudience(value: unknown): boolean {
  if (!isPlainObject(value)) return false;
  if (!(SOVEREIGN_LICENSE_TERMS_AUDIENCE_KINDS as readonly unknown[]).includes(value.kind)) return false;

  if (value.kind === SovereignLicenseTermsAudienceKind.Public) {
    return hasExactKeys(value, ['kind']);
  }
  if (value.kind === SovereignLicenseTermsAudienceKind.Principal) {
    return hasExactKeys(value, ['kind', 'principal']) && isStructuralPrincipal(value.principal);
  }
  return (
    hasExactKeys(value, ['kind', 'namespace', 'id'])
    && isNonBlankString(value.namespace)
    && isNonBlankString(value.id)
  );
}

export function isValidSovereignLicenseTermsAudience(value: unknown): value is SovereignLicenseTermsAudience {
  return validateSovereignLicenseTermsAudience(value);
}

export function isValidSovereignLicenseTermsRuleEffect(
  value: unknown,
): value is SovereignLicenseTermsRuleEffect {
  return (
    typeof value === 'string'
    && (SOVEREIGN_LICENSE_TERMS_RULE_EFFECTS as readonly string[]).includes(value)
  );
}
 ...
 */
export function validateSovereignLicenseActionRef(value: unknown): readonly LicensingTermsReasonCode[] {
  ...
  if (value.namespace === AOC_LICENSING_SEMANTIC_NAMESPACE && !isAocLicensingActionTermId(value.termRef)) {
    return [codes.unknownAocAction];
  }
  return [];
}

export function isValidSovereignLicenseActionRef(value: unknown): value is SovereignLicenseActionRef {
  return validateSovereignLicenseActionRef(value).length === 0;
}

/** Validates one rule clause, closed to exactly its four keys. */
export function validateSovereignLicenseTermsRuleV1(value: unknown): readonly LicensingTermsReasonCode[] {
  ...
  if (!isNonBlankString(value.id)) reasons.push(codes.invalidRule);
  if (!isValidSovereignLicenseTermsRuleEffect(value.effect)) reasons.push(codes.invalidRuleEffect);
  reasons.push(...validateSovereignLicenseActionRef(value.action));
  if (!isNonBlankString(value.statement)) reasons.push(codes.invalidRule);
  return distinct(reasons);
}

export function isValidSovereignLicenseTermsRuleV1(value: unknown): value is SovereignLicenseTermsRuleV1 {
 ...
 */
export function validateSovereignLicenseTermsV1(value: unknown): readonly LicensingTermsReasonCode[] {
  if (!isPlainObject(value)) return [codes.invalidInput];

  const expected = hasOwn(value, 'effectiveAt')
    ? ['schemaVersion', 'audience', 'effectiveAt', 'rules']
    : ['schemaVersion', 'audience', 'rules'];
  if (!hasExactKeys(value, expected)) return [codes.invalidInput];
  ...
  if (value.schemaVersion !== SOVEREIGN_LICENSE_TERMS_SCHEMA_VERSION) {
    reasons.push(codes.unsupportedSchema);
  }

  if (!validateSovereignLicenseTermsAudience(value.audience)) reasons.push(codes.invalidAudience);
...
export function isValidSovereignLicenseTermsV1(value: unknown): value is SovereignLicenseTermsV1 {
