...
exports.isVerificationResultCurrentForRevision = isVerificationResultCurrentForRevision;
exports.listVerificationResultsForRevision = listVerificationResultsForRevision;
exports.listLatestVerificationResults = listLatestVerificationResults;
exports.countVerificationOutcomes = countVerificationOutcomes;
 ...
 * to APV-09.
 */
function isVerificationResultCurrentForRevision(result, caseRevision) {
    return result.evaluatedCaseRevision === caseRevision;
}
/** The results that evaluated exactly this case revision, in execution order. */
function listVerificationResultsForRevision(results, caseRevision) {
    return [...results]
        .filter((result) => isVerificationResultCurrentForRevision(result, caseRevision))
        .sort(verification_repository_1.compareVerificationResults);
}
function keyOf(result) {
    // A requirement id is a dotted token and cannot contain `\n`, so the separator
    // is never ambiguous between two distinct pairs.
 ...
 */
function listLatestVerificationResults(results) {
    const latest = new Map();
    for (const result of results) {
        const key = keyOf(result);
        const held = latest.get(key);
        if (held === undefined || (0, verification_repository_1.compareVerificationResults)(held, result) < 0) {
            latest.set(key, result);
        }
    }
    return [...latest.values()].sort(verification_repository_1.compareVerificationResults);
}
/**
 ...
 */
function countVerificationOutcomes(results) {
    const counts = {};
    for (const outcome of verification_outcome_1.VERIFICATION_CHECK_OUTCOMES)
        counts[outcome] = 0;
    for (const result of results)
