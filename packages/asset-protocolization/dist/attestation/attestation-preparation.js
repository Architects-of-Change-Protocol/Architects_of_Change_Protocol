...
exports.isUsableProofRef = isUsableProofRef;
exports.prepareCanonicalAttestationFromReview = prepareCanonicalAttestationFromReview;
 ...
 */
function isUsableProofRef(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        return false;
    const candidate = value;
    if (!isCanonicalRecordId(candidate.id))
        return false;
    if (typeof candidate.type !== 'string' || !Object.values(claims_1.ProofType).includes(candidate.type)) {
        return false;
    }
...
function refuse(message, reasonCodes) {
    throw new review_errors_1.ProfessionalReviewError(review_errors_1.PROFESSIONAL_REVIEW_ERROR_CODES.attestationCannotBeConstructed, message, { reasonCodes });
}
/**
...
function prepareCanonicalAttestationFromReview(input) {
    ...
    if (reasons.length > 0) {
        refuse(`Refusing to construct a CanonicalAttestation from incomplete inputs: ${reasons.join(', ')}`, reasons);
    }
    return {
