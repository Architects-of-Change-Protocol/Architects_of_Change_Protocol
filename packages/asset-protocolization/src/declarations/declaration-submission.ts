 ...
 * reads.
 */
export const DeclarationPathway = {
  /**
...
export interface CanonicalClaimDeclarationSubmission
  extends ProtocolizationDeclarationSubmissionBase {
...
export type ProtocolizationDeclarationSubmission =
  | ReferencedDeclarationSubmission
  | CanonicalClaimDeclarationSubmission;

/** The keys every pathway shares. */
export const DECLARATION_SUBMISSION_BASE_KEYS: readonly string[] = [
  'declarationId',
  'caseId',
  'materialId',
  'declarant',
...
const PATHWAY_KEYS: Readonly<Record<DeclarationPathway, readonly string[]>> = Object.freeze({
  [DeclarationPathway.Reference]: Object.freeze(['claimRef', 'claimType']),
  [DeclarationPathway.Canonical]: Object.freeze(['claim']),
});

export function declarationPathwayKeys(pathway: DeclarationPathway): readonly string[] {
  return PATHWAY_KEYS[pathway];
}

export function isDeclarationPathway(value: unknown): value is DeclarationPathway {
  return typeof value === 'string' && Object.values<string>(DeclarationPathway).includes(value);
}
 ...
 */
export function submittedClaimRef(
  submission: ProtocolizationDeclarationSubmission,
): CanonicalClaimId {
  return submission.pathway === DeclarationPathway.Canonical
    ? submission.claim.id
    : submission.claimRef;
}
 ...
 */
export function submittedClaimType(
  submission: ProtocolizationDeclarationSubmission,
): ClaimType {
  return submission.pathway === DeclarationPathway.Canonical
