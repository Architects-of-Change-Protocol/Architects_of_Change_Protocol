...
import { validateProtocolizationDeclarationRecord } from './declaration-validation';
...
export interface DeclarationRepository {
  ...
  ): AdapterResult<ProtocolizationDeclarationRecord | undefined>;
  ...
  ): AdapterResult<readonly ProtocolizationDeclarationRecord[]>;
...
function assertTenant(tenantId: ProtocolizationTenantId): void {
  if (!isValidProtocolizationTenantId(tenantId)) {
    throw new DeclarationError(
      ...
      'A non-blank tenantId is required to address a ProtocolizationDeclarationRecord',
 ...
 */
export function createInMemoryDeclarationRepository(): DeclarationRepository {
  const entries = new Map<string, ProtocolizationDeclarationRecord>();
  ...
  return {
    ...
    listByCase(tenantId, caseId) {
      ...
      return [...entries.values()].filter(
        (record) => record.tenantId === tenantId && record.caseId === caseId,
      );
    },

    save(record) {
      const validation = validateProtocolizationDeclarationRecord(record);
      if (!validation.admitted) {
        throw new DeclarationError(
          DECLARATION_ERROR_CODES.invalidRecord,
          `Refusing to store an invalid ProtocolizationDeclarationRecord: ${validation.reasons.join(', ')}`,
      ...
      if (entries.has(key)) {
        throw new DeclarationError(
          DECLARATION_ERROR_CODES.duplicateDeclaration,
          'A ProtocolizationDeclarationRecord with this (tenantId, declarationId) already exists',
          {
            reasonCodes: [DECLARATION_ERROR_CODES.duplicateDeclaration],
      ...
      entries.set(key, deepFreeze(record));
    },
  };
}
