...
import { validateEvidenceIntakeReceipt } from './evidence-intake-validation';
...
export interface EvidenceIntakeRepository {
  ...
  get(
    ...
    intakeId: EvidenceIntakeId,
  ): AdapterResult<EvidenceIntakeReceipt | undefined>;
  ...
  ): AdapterResult<readonly EvidenceIntakeReceipt[]>;
...
function storageKey(tenantId: ProtocolizationTenantId, intakeId: EvidenceIntakeId): string {
...
function assertTenant(tenantId: ProtocolizationTenantId): void {
  if (!isValidProtocolizationTenantId(tenantId)) {
    throw new EvidenceIntakeError(
      EVIDENCE_INTAKE_ERROR_CODES.invalidTenant,
      'A non-blank tenantId is required to address an EvidenceIntakeReceipt',
 ...
 */
export function createInMemoryEvidenceIntakeRepository(): EvidenceIntakeRepository {
  const entries = new Map<string, EvidenceIntakeReceipt>();
  ...
  return {
    ...
    listByCase(tenantId, caseId) {
      ...
      return [...entries.values()].filter(
        (receipt) => receipt.tenantId === tenantId && receipt.caseId === caseId,
      );
    },

    save(receipt) {
      const validation = validateEvidenceIntakeReceipt(receipt);
      if (!validation.admitted) {
        throw new EvidenceIntakeError(
          EVIDENCE_INTAKE_ERROR_CODES.invalidReceipt,
          `Refusing to store an invalid EvidenceIntakeReceipt: ${validation.reasons.join(', ')}`,
      ...
      if (entries.has(key)) {
        throw new EvidenceIntakeError(
          EVIDENCE_INTAKE_ERROR_CODES.duplicateIntake,
          'An EvidenceIntakeReceipt with this (tenantId, intakeId) already exists',
          {
            reasonCodes: [EVIDENCE_INTAKE_ERROR_CODES.duplicateIntake],
      ...
      entries.set(key, deepFreeze(receipt));
    },
  };
}
