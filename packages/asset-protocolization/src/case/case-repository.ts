...
import { isValidProtocolizationTenantId } from './case-identifiers';
import type { ProtocolizationCaseId, ProtocolizationTenantId } from './case-identifiers';
import { validateProtocolizationCase } from './case-validation';
...
export interface ProtocolizationCaseRepository {
  ...
  ): AdapterResult<ProtocolizationCase | undefined>;
...
function storageKey(tenantId: ProtocolizationTenantId, caseId: ProtocolizationCaseId): string {
...
function assertTenant(tenantId: ProtocolizationTenantId): void {
  if (!isValidProtocolizationTenantId(tenantId)) {
    throw new ProtocolizationCaseError(
      ...
      'A non-blank tenantId is required to address a ProtocolizationCase',
      { reasonCodes: [PROTOCOLIZATION_CASE_ERROR_CODES.invalidTenant] },
    );
  }
}
 ...
 */
export function createInMemoryProtocolizationCaseRepository(): ProtocolizationCaseRepository {
  const entries = new Map<string, ProtocolizationCase>();
    ...
    save(protocolizationCase) {
      ...
      if (!validation.valid) {
        throw new ProtocolizationCaseError(
          ...
          `Refusing to store an invalid ProtocolizationCase: ${validation.reasons.join(', ')}`,
      ...
      if (stored === undefined) {
        if (protocolizationCase.revision !== 1) {
          throw new ProtocolizationCaseError(
            ...
            `No stored ProtocolizationCase to update at revision ${protocolizationCase.revision}`,
            {
              reasonCodes: [PROTOCOLIZATION_CASE_ERROR_CODES.caseNotFound],
        ...
        throw new ProtocolizationCaseError(
          ...
          'A ProtocolizationCase with this (tenantId, caseId) already exists',
          {
            reasonCodes: [PROTOCOLIZATION_CASE_ERROR_CODES.duplicateCase],
            caseId: protocolizationCase.caseId,
            tenantId: protocolizationCase.tenantId,
          },
        );
        ...
        throw new ProtocolizationCaseError(
          ...
          'This ProtocolizationCase was modified by another writer',
          {
            reasonCodes: [PROTOCOLIZATION_CASE_ERROR_CODES.revisionConflict],
            caseId: protocolizationCase.caseId,
            tenantId: protocolizationCase.tenantId,
      ...
      entries.set(key, deepFreeze(protocolizationCase));
    },
  };
}
