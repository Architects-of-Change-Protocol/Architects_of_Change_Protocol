import { isValidAssetVerificationCheckId } from '../identifiers';
...
function failRegistration(message: string, checkId?: AssetVerificationCheckId): never {
  throw new VerificationError(
    VERIFICATION_ERROR_CODES.checkRegistrationInvalid,
    message,
    {
      reasonCodes: [VERIFICATION_ERROR_CODES.checkRegistrationInvalid],
 ...
 */
export function createVerificationCheckRegistry(
  checks: readonly AssetVerificationCheck[] = [],
): VerificationCheckRegistry {
  const entries = new Map<AssetVerificationCheckId, AssetVerificationCheck>();

  const registry: VerificationCheckRegistry = {
    register(check) {
      if (typeof check !== 'object' || check === null) {
        failRegistration('A verification check implementation is required');
      }
      if (!isValidAssetVerificationCheckId(check.checkId)) {
        failRegistration('A verification check must carry a well-formed AssetVerificationCheckId');
      }
      if (typeof check.execute !== 'function') {
        failRegistration(
          `Verification check ${check.checkId} does not implement execute()`,
          check.checkId,
        );
      }
      if (entries.has(check.checkId)) {
        failRegistration(
          `A verification check is already registered for ${check.checkId}`,
          check.checkId,
        );
      }
  ...
  for (const check of checks) registry.register(check);
  return registry;
}
