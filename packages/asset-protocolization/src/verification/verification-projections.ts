...
import { VERIFICATION_CHECK_OUTCOMES } from './verification-outcome';
import type { VerificationCheckOutcome } from './verification-outcome';
import { compareVerificationResults } from './verification-repository';
 ...
 * to APV-09.
 */
export function isVerificationResultCurrentForRevision(
...
export function listVerificationResultsForRevision(
...
function keyOf(result: ProtocolizationVerificationResult): string {
 ...
 */
export function listLatestVerificationResults(
  results: readonly ProtocolizationVerificationResult[],
): readonly ProtocolizationVerificationResult[] {
  const latest = new Map<string, ProtocolizationVerificationResult>();
  for (const result of results) {
    const key = keyOf(result);
    const held = latest.get(key);
    if (held === undefined || compareVerificationResults(held, result) < 0) {
      latest.set(key, result);
    }
  }
  return [...latest.values()].sort(compareVerificationResults);
}
 ...
 */
export function countVerificationOutcomes(
  results: readonly ProtocolizationVerificationResult[],
): VerificationOutcomeCounts {
  const counts: Record<string, number> = {};
  for (const outcome of VERIFICATION_CHECK_OUTCOMES) counts[outcome] = 0;
  for (const result of results) counts[result.outcome] = (counts[result.outcome] ?? 0) + 1;
  return Object.freeze(counts) as VerificationOutcomeCounts;
}
