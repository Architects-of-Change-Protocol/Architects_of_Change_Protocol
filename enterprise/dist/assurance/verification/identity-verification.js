...
exports.IdentityVerificationEngine = void 0;
class IdentityVerificationEngine {
    verify(input, state) {
        const now = input.now ?? new Date();
        const candidates = [...state.credentials].filter((credential) => credential.subject_hash === input.subject_hash &&
        ...
        if (input.consumer_id !== undefined) {
            const consentExists = [...state.consents].some((consent) => consent.subject_hash === input.subject_hash &&
...
exports.IdentityVerificationEngine = IdentityVerificationEngine;
