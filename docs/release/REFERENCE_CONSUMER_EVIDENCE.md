...
| Chain of custody | The same SHA-256 is produced by a fresh `npm pack` from this repository today (recorded in [`evidence/aoc-protocol-0.1.0-release-manifest.json`](evidence/aoc-protocol-0.1.0-release-manifest.json)); Enterprise's evidence additionally records three independent rebuilds from the pinned commit with identical SHA-256 |
