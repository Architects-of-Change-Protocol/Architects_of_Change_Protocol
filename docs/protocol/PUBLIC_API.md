...
| `@aoc/protocol/sovereignty-capabilities` | `SOVEREIGNTY_CAPABILITY_NAMESPACE`, `SOVEREIGNTY_CAPABILITY_KEYS`, `SOVEREIGNTY_CAPABILITY_IDS`, `SOVEREIGNTY_CAPABILITIES`, `listSovereigntyCapabilities`, `getSovereigntyCapability`, `getSovereigntyCapabilityByKey`, `isSovereigntyCapabilityId`, `isSovereigntyCapabilityKey`, `SovereigntyCapabilityId`, `SovereigntyCapabilityKey`, `SovereigntyCapabilityNamespace`, `SovereigntyCapabilityVersion`, `SovereigntyCapabilityDefinition` | Stable | Mixed — frozen constant data plus pure lookup functions | Added post-0.1.0 | Canonical identity, version and discovery for the eight Sovereignty Capabilities. Read-only: no registration, mutation or invocation API. Zero dependencies (not even `node:crypto`), so it is browser-safe. See `docs/protocol/SOVEREIGNTY_CAPABILITIES.md` |
